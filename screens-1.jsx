...
function ApprovalScreen({ onOpen }) {
  ...
  return (
    <div className="stage" style={{maxWidth: 880, paddingTop: 56, paddingBottom: 160}}>
      <button onClick={() => onOpen('today')} className="mono" style={{
      ...
      <div className="mono" style={{
        ...
        <span>APPROVAL · NEVER AUTO · BKG-44188</span>
        <span style={{flex:1}} />
        <span style={{display:'inline-flex', alignItems:'center', gap:6, color:'var(--warn)'}}>
          ...
          OTA WINDOW {v2.sla.remaining_h}H · CLOSES IN {v2.sla.remaining_h}H
        </span>
      </div>

      {/* HERO — Cendra's recommendation in big serif */}
      <div style={{marginBottom: 40}}>
        <h1 className="serif-display" style={{
          fontSize: 42, lineHeight: 1.07, margin: 0, color: 'var(--ink)',
        }}>
          {a.title}
        </h1>
        <p style={{
          fontSize: 17, lineHeight: 1.55, margin: '18px 0 0',
          color: 'var(--ink-mid)', maxWidth: 740, fontFamily: 'var(--sans)',
        }}>
          Cendra recommends sending a <b style={{color:'var(--ink)'}}>neutral message first</b>, filing the formal claim only if the guest goes silent. The OTA window has 38h — enough buffer for a softer first contact.
        </p>
      </div>

      {/* HERO ACTION CARD — Safer alternative is primary, Authorize is secondary */}
      <div style={{
        ...
        <div style={{display:'flex', alignItems:'center', gap: 10, marginBottom: 14}}>
          <span className="mono" style={{fontSize: 10, letterSpacing: '.18em', color: 'var(--ok)', fontWeight: 600}}>CENDRA RECOMMENDS</span>
          <span style={{width:3, height:3, borderRadius:'50%', background:'var(--muted-2)'}} />
          <span className="mono" style={{fontSize:10, letterSpacing:'.12em', color:'var(--muted)'}}>SAFER ALTERNATIVE · REVERSIBLE</span>
        </div>
        <h2 className="serif-display" style={{fontSize: 24, lineHeight: 1.2, margin: 0, color:'var(--ink)', marginBottom: 12, maxWidth: 720}}>
          Send a neutral first contact. Keep the claim option open.
        </h2>
        <p style={{margin: 0, fontSize: 14.5, lineHeight: 1.6, color:'var(--ink-mid)', maxWidth: 720}}>
          {v2.safer_alternative.body}
        </p>
        <div style={{display:'flex', alignItems:'center', gap: 12, marginTop: 20, flexWrap:'wrap'}}>
          ...
          <Btn kind="ghost" onClick={() => setAuthorizeOpen(o => !o)}>Authorize €640 claim instead →</Btn>
          <span style={{flex:1}} />
          <Btn kind="ghost" size="sm" onClick={() => setEscalateOpen(true)}>Escalate</Btn>
        </div>
      </div>

      {/* CONSEQUENCE STRIP — micro */}
      <div style={{
        display:'flex', gap: 36, flexWrap:'wrap',
        paddingBottom: 24, marginBottom: 24, borderBottom:'1px solid var(--hair-soft)',
      }}>
        <MicroStatBlock2 value="€640" label="charge" tone="risk" />
        <MicroStatBlock2 value="Booking.com" label="channel" />
        <MicroStatBlock2 value="38h" label="ota window" tone="warn" />
        <MicroStatBlock2 value="Final" label="reversibility" tone="risk" />
        <MicroStatBlock2 value="Bosphorus" label="owner" />
      </div>

      {/* AUTHORIZE GATE — collapsible, hidden behind confirmation */}
      {authorizeOpen && (
        <div style={{
          background:'#ffffff', border:'1px solid var(--rausch)',
          borderRadius: 16, padding:'24px 28px', marginBottom: 32,
          boxShadow:'0 4px 24px rgba(255,56,92,0.06)',
        }}>
          <div className="mono" style={{fontSize: 10, letterSpacing:'.18em', color:'var(--rausch)', fontWeight: 600, marginBottom: 14}}>AUTHORIZE CLAIM · NOT REVERSIBLE</div>
          <ul style={{margin: 0, paddingLeft: 20, fontSize: 13.5, lineHeight: 1.7, color:'var(--ink-mid)'}}>
            <li>Charge €640 via Booking.com Resolution Center</li>
            <li>Send message · {a.drafts[draftIdx].tone}</li>
            <li>Log incident in audit trail · immutable</li>
            <li>Notify Bosphorus Holdings via email</li>
          </ul>
          <div className="mono" style={{fontSize: 10.5, color:'var(--muted)', letterSpacing:'.14em', marginTop: 18, marginBottom: 8}}>TYPE "{phrase}" TO PROCEED</div>
          <input
            type="text"
            value={confirmText}
            ...
            style={{
              ...
              border:'1px solid ' + (ok ? 'var(--ok)' : 'var(--hair)'),
              borderRadius: 8, outline: 'none', background:'#ffffff',
              fontFamily:'var(--sans)', color:'var(--ink)',
            }}
          />
          <div style={{display:'flex', gap: 10, marginTop: 16, alignItems:'center'}}>
            <button disabled={!ok} onClick={() => alert('Claim authorized')} style={{
              all:'unset', cursor: ok ? 'pointer' : 'not-allowed',
              background: ok ? 'var(--rausch)' : 'var(--hair)',
              color: ok ? '#ffffff' : 'var(--muted)',
              padding:'12px 22px', borderRadius: 10,
              fontSize: 14.5, fontWeight: 600,
              display:'inline-flex', alignItems:'center', gap: 8,
              opacity: ok ? 1 : 0.6,
            ...
            <Btn kind="ghost" onClick={() => { setAuthorizeOpen(false); setConfirmText(""); }}>Cancel</Btn>
          </div>
        </div>
      )}
      ...
      <CollapsibleStep
        eyebrow={`EVIDENCE PACK · ${a.evidence.length} ITEMS · ALL FRESH`}
        sub="Cendra gathered photos, facts, rules, and policies."
        open={evidenceOpen}
        ...
        <div className="dcard" style={{padding: 0, overflow:'hidden', marginTop: 14}}>
          {a.evidence.map((e, i) => (
            <div key={i} style={{
              ...
              gap: 16, padding:'14px 20px', alignItems:'flex-start',
              borderBottom: i < a.evidence.length - 1 ? '1px solid var(--hair-soft)' : 'none',
              background:'#ffffff',
            }}>
              <div className="mono" style={{fontSize: 10, letterSpacing:'.14em', color:'var(--ink)', fontWeight: 600, paddingTop: 2}}>{e.kind.toUpperCase()}</div>
              <div>
                <div style={{fontSize: 13.5, color:'var(--ink)', lineHeight: 1.5}}>{e.label}</div>
                <div className="mono" style={{fontSize: 10.5, color:'var(--muted)', letterSpacing:'.04em', marginTop: 4}}>{e.source}</div>
              </div>
              <div className="mono" style={{fontSize: 10.5, color:'var(--muted)', letterSpacing:'.06em', textAlign:'right'}}>{e.fresh}</div>
            </div>
          ))}
        </div>
      </CollapsibleStep>
      ...
      <CollapsibleStep
        eyebrow={`PROPOSED MESSAGE · ${a.drafts[draftIdx].tone.toUpperCase()}`}
        sub="Tone-switchable. Cendra holds this until you decide."
        open={draftOpen}
        onToggle={() => setDraftOpen(o => !o)}
      >
        <div className="dcard" style={{padding: '18px 22px', marginTop: 14}}>
          <p style={{
            margin: 0, fontFamily:'var(--serif)', fontSize: 16, lineHeight: 1.55,
            color: 'var(--ink)', fontStyle:'italic',
          }}>
            "{a.drafts[draftIdx].body}"
          </p>
          <div style={{display:'flex', gap: 6, marginTop: 16, flexWrap:'wrap'}}>
            {a.drafts.map((d, i) => (
              <button key={i} onClick={() => setDraftIdx(i)} style={{
                all:'unset', cursor:'pointer',
                padding:'5px 12px', borderRadius: 999,
                border:'1px solid ' + (draftIdx === i ? 'var(--ink)' : 'var(--hair)'),
                background: draftIdx === i ? 'var(--ink)' : '#ffffff',
                color: draftIdx === i ? '#ffffff' : 'var(--ink-mid)',
      ...
      {/* IF YOU DECLINE — quiet note */}
      <div style={{marginTop: 40, paddingTop: 24, borderTop: '1px solid var(--hair-soft)'}}>
        <div className="mono" style={{fontSize: 10, letterSpacing:'.14em', color:'var(--muted)', textTransform:'uppercase', marginBottom: 8, fontWeight: 500}}>IF YOU DO NOTHING</div>
        <p style={{margin: 0, fontSize: 13.5, lineHeight: 1.55, color:'var(--ink-mid)', maxWidth: 700, fontStyle:'italic', fontFamily:'var(--serif)'}}>
          {v2.decline_consequence}
        </p>
      </div>

      {/* APPROVERS — compact */}
      <div style={{marginTop: 40}}>
        <div className="mono" style={{fontSize: 10, letterSpacing:'.14em', color:'var(--muted)', textTransform:'uppercase', marginBottom: 12, fontWeight: 500}}>APPROVERS</div>
          ...
          {v2.approvers.map((p, i) => (
            <div key={i} style={{
              ...
              <div style={{flex:1, minWidth: 0}}>
                <div className="mono" style={{fontSize: 10, letterSpacing:'.14em', color:'var(--muted)', textTransform:'uppercase', marginBottom: 2}}>{p.role}</div>
                <div style={{fontSize: 13.5, fontWeight: 500, color:'var(--ink)'}}>{p.name}</div>
              </div>
              {p.state === 'current'
                ? <Pill tone="warn">You · pending</Pill>
                : <Btn size="sm" kind="ghost" onClick={() => setEscalateOpen(true)}>Escalate →</Btn>
              }
            </div>
          ))}
        </div>
      ...
      <div className="mono" style={{
        fontSize: 10, letterSpacing:'.12em', color:'var(--muted-2)',
